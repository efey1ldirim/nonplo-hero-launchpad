  ...
  return <section className="py-20 bg-background relative" id="pricing">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => <div key={index} className={`relative bg-card rounded-2xl p-8 border transition-all duration-300 hover:-translate-y-1 ${plan.popular ? "border-primary shadow-primary/20 shadow-lg scale-105" : "border-border shadow-card hover:shadow-primary/10"}`}>
              {/* Popular badge */}
              ...
              <div className="text-center mb-8">
                ...
                <div className="mb-4">
                  <div className="flex items-baseline justify-center gap-1">
                    {plan.price === 0 ? <span className="text-4xl font-bold text-foreground">
                        Ücretsiz
                      </span> : <>
              ...
              {/* CTA button */}
              <Button variant={plan.popular ? "hero" : "outline"} className="w-full text-base py-6 h-auto">
                {plan.cta}
              </Button>
            </div>)}
        </div>
